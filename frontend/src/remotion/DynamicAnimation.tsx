...
const Particle: React.FC<{
  ...
  color: string;
  duration: number;
}> = ({ delay, angle, distance, size, color, duration: _duration }) => {
  void _duration;
...
const CameraWrapper: React.FC<{
  ...
  const progress = interpolate(frame, [0, safeDuration], [0, 1], { extrapolateRight: 'clamp' });
...
const TransitionWrapper: React.FC<{
...
}> = ({ children, transitionType, transitionDuration = 15, isEntry: _isEntry = true }) => {
  void _isEntry;
  const frame = useCurrentFrame();
  ...
  const entryEnd = Math.min(transitionDuration, safeDuration);
...
case 'row': {
            ...
            x = 50 - (emojis.length - 1) * spacing / 2 + index * spacing;
            y = 50;
            break;
          }
        }
...
const AnimatedGifItem: React.FC<{ gifConfig: GifConfig; index: number }> = ({ gifConfig, index }) => {
  ...
  let animationStyle = {};
  const _animFrame = frames - delay;

  switch (animation) {
    case 'pop': {
      const popScale = interpolate(
        Math.sin(_animFrame * 0.1) * 0.5 + 0.5,
        [0, 1],
        [0.95, 1.05]
      );
...
export const DynamicAnimation: React.FC<DynamicAnimationProps> = ({
  ...
  // Validate scenes - ensure no zero-duration scenes that would crash Remotion's interpolate
  const safeScenes = scenes.map((scene) => ({
    ...scene,
    duration: Math.max(1, scene.duration || 30),
  }));
