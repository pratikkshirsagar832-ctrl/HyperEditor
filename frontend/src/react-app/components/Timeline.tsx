...
import { ZoomIn, ZoomOut, Play, Pause, SkipBack, Scissors, Trash2, Type, RectangleHorizontal, RectangleVertical, Link, Unlink, Lock, Unlock as UnlockIcon, Flag } from 'lucide-react';
import TimelineClip from './TimelineClip';
import MarkerModal from './MarkerModal';
import type { Track, TimelineClip as TimelineClipType, Asset, CaptionData, Marker } from '@/react-app/hooks/useProject';
...
export default function Timeline({
  ...
  }, [pixelsPerSecond, onDropAsset, tracks]);
  ...
  const waveformLoadingRef = useRef<Set<string>>(new Set());

  // Fetch waveform data for audio clips
  useEffect(() => {
    const audioClips = clips.filter(c => {
      const asset = assets.find(a => a.id === c.assetId);
      return asset?.type === 'audio' && !waveformData[c.id] && !waveformLoadingRef.current.has(c.id);
    });

    if (audioClips.length === 0) return;

    const sampleAudio = assets.find(a => audioClips.some(c => c.assetId === a.id) && a.type === 'audio');
    if (!sampleAudio?.streamUrl) return;

    const sessionMatch = sampleAudio.streamUrl.match(/\/session\/([^/]+)\/assets/);
    ...
    for (const clip of audioClips) {
      ...
      fetch(`${LOCAL_FFMPEG_URL}/session/${sessionId}/assets/${asset.id}/waveform`, {
        ...
        .then(data => {
          ...
          if (data.peaks && Array.isArray(data.peaks)) {
            setWaveformData(prev => ({ ...prev, [clip.id]: data.peaks }));
          }
        })
        .catch(() => {
          waveformLoadingRef.current.delete(clip.id);
  ...
  return (
    ...
    >
                  ...
                  <button
                    onClick={() => {
                      const updated = tracks.map(t =>
                        t.id === track.id ? { ...t, isLocked: !t.isLocked } : t
                      );
                      onTracksChange?.(updated);
                    }}
                    className={`p-0.5 rounded transition-colors flex-shrink-0 ${
                      track.isLocked ? 'text-red-400' : 'text-zinc-500 hover:text-zinc-300'
                    }`}
                    title={track.isLocked ? 'Unlock track' : 'Lock track'}
                  >
                    {track.isLocked ? <Lock size={12} /> : <UnlockIcon size={12} />}
                  </button>
        ...
        >
          ...
          >
                    ...
                    {trackClips.map(clip => {
                      ...
                      return (
                        <TimelineClip
                          ...
                          waveformPeaks={waveformData[clip.id]}
                        />
                      );
                    })}
                  </div>
            ...
            {markers.map((marker) => (
              <div
                key={marker.id}
                className="absolute top-0 bottom-0 w-0.5 cursor-pointer z-30 group"
                style={{
                  left: `${(marker.time / totalDuration) * timelineWidth}px`,
                  backgroundColor: {
                    red: '#EF4444',
                    yellow: '#EAB308',
                    green: '#22C55E',
              ...
              >
                <div className="absolute -top-1 -left-1 w-2 h-2 rounded-full bg-current opacity-0 group-hover:opacity-100 transition-opacity" />
              </div>
            ))}
          </div>
        </div>
      ...
      <MarkerModal
        ...
        onAddMarker={(m) => {
          const newMarker: Marker = { ...m, id: crypto.randomUUID() };
