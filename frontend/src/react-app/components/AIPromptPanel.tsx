...
export default function AIPromptPanel({
  ...
  const PRESET_COMMANDS = [
    { label: 'Add Intro', prompt: 'Add a 3-second animated intro with channel name at the start of the video', icon: '🎬' },
    { label: 'Add Outro', prompt: 'Add a 5-second outro with subscribe button at the end of the video', icon: '🎬' },
    { label: 'Remove Dead Air', prompt: 'Remove all silence and gaps from the video to make it faster', icon: '✂️' },
    { label: 'Add Captions', prompt: 'Transcribe the video and add auto-generated captions', icon: '📝' },
    { label: 'Add B-Roll', prompt: 'Add relevant B-roll images between segments to enhance the video', icon: '🖼️' },
    { label: 'Speed Up', prompt: 'Find slow parts of the video and speed them up appropriately', icon: '⚡' },
  ];
  ...
  const pollForResult = async (
    jobId: string,
    maxAttempts = 60
  ): Promise<{ success: boolean; command: string; explanation: string; error?: string }> => {
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      setProcessingStatus(`AI is working... (${attempt + 1}s)`);

      try {
        const response = await fetch(`${LOCAL_FFMPEG_URL}/api/ai-edit/status/${jobId}`);
        if (!response.ok) {
  ...
  return (
    ...
    >
      ...
      {/* Preset commands (shown when chat is empty) */}
      {chatHistory.length === 0 && hasVideo && (
        <div className="px-4 pt-4 pb-1">
          <p className="text-[10px] text-zinc-500 uppercase tracking-wider mb-2">Quick Commands</p>
          <div className="grid grid-cols-2 gap-1.5">
            {PRESET_COMMANDS.map((cmd) => (
              <button
                ...
                className="flex items-center gap-1.5 px-2.5 py-2 bg-zinc-800/60 hover:bg-zinc-700/80 rounded-lg text-xs text-zinc-300 transition-colors text-left border border-zinc-700/30"
              >
      ...
      <div className="flex-1 p-4 overflow-y-auto space-y-4">
        {chatHistory.length === 0 ? (
          <div className="text-center text-sm text-zinc-500 py-4">
            {hasVideo
              ? "Describe your edit above or use Quick Commands"
