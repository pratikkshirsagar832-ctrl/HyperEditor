...
import ClipifyPanel from '@/react-app/components/ClipifyPanel';
import { HyperFramesPanel } from '@/react-app/components/HyperFramesPanel';
...
import { useProject, setUndoRedoListener, LOCAL_FFMPEG_URL, Asset, TimelineClip, CaptionStyle } from '@/react-app/hooks/useProject';
import { useVideoSession } from '@/react-app/hooks/useVideoSession';
import { useJobProgress } from '@/react-app/hooks/useJobProgress';
import { Sparkles, ListOrdered, Copy, Check, X, Download, Play, Palette, Film, Scissors } from 'lucide-react';
import type { TemplateId } from '@/remotion/templates';

const FFMPEG_URL = LOCAL_FFMPEG_URL;
...
export default function Home() {
  ...
  const [activeAgent, setActiveAgent] = useState<'director' | 'picasso' | 'dicaprio' | 'clipify' | 'hyperframes'>('director');
  ...
  const { activeJob, showProgress, cancelJob } = useJobProgress(session?.sessionId ?? null);
  ...
  // Register undo toast listener
  useEffect(() => {
    setUndoRedoListener((msg: string) => {
      setUndoToast(msg);
      setTimeout(() => setUndoToast(null), 1500);
    });
    return () => setUndoRedoListener(null);
  }, []);

  // Global keyboard shortcuts
  // Use refs for hot-path values to avoid re-registering listeners every frame
  ...
  }, [selectedClipId, clips, currentTime, activeTabId]);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if ((e.key === 'Delete' || e.key === 'Backspace') && selectedClipIdRef.current) {
        ...
        if (activeTabIdRef.current !== 'main') {
          const activeTab = timelineTabs.find(tab => tab.id === activeTabIdRef.current);
          if (activeTab) {
            const updatedClips = activeTab.clips.filter(c => c.id !== id);
            updateTabClips(activeTabIdRef.current, updatedClips);
          }
        } else {
          deleteClip(id, autoSnap);
        }
      ...
      // ── Nudge selected clip ────────────────────────────────────
      if ((e.key === 'ArrowLeft' || e.key === 'ArrowRight') && selectedClipIdRef.current) {
        e.preventDefault();
        const delta = e.key === 'ArrowLeft' ? -0.1 : 0.1;
        const clip = clipsRef.current.find(c => c.id === selectedClipIdRef.current);
        if (clip && activeTabIdRef.current === 'main') {
          moveClip(selectedClipIdRef.current, Math.max(0, clip.start + delta));
          saveProject();
        } else if (clip && activeTabIdRef.current !== 'main') {
          const activeTab = timelineTabs.find(t => t.id === activeTabIdRef.current);
          if (activeTab) {
            const updatedClips = activeTab.clips.map(c =>
              c.id === selectedClipIdRef.current ? { ...c, start: Math.max(0, c.start + delta) } : c
            );
            updateTabClips(activeTabIdRef.current, updatedClips);
          }
        }
      ...
      // ── In/Out trim points ─────────────────────────────────────
      if (e.key === 'i' && selectedClipIdRef.current) {
        e.preventDefault();
        const clip = clipsRef.current.find(c => c.id === selectedClipIdRef.current);
        if (clip && activeTabIdRef.current === 'main') {
          const timeInClip = currentTimeRef.current - clip.start;
          if (timeInClip >= 0 && timeInClip < clip.outPoint - clip.inPoint) {
            updateClip(selectedClipIdRef.current, { inPoint: clip.inPoint + timeInClip, duration: clip.outPoint - (clip.inPoint + timeInClip) });
          }
        }
        return;
      }
      if (e.key === 'o' && selectedClipIdRef.current) {
        e.preventDefault();
        const clip = clipsRef.current.find(c => c.id === selectedClipIdRef.current);
        if (clip && activeTabIdRef.current === 'main') {
          const timeInClip = currentTimeRef.current - clip.start;
          if (timeInClip > clip.inPoint && timeInClip < clip.outPoint) {
            updateClip(selectedClipIdRef.current, { outPoint: clip.inPoint + timeInClip, duration: timeInClip });
          }
        }
        return;
      }
  ...
  }, [undo, redo, moveClip, timelineTabs, updateTabClips, updateClip, saveProject, deleteClip, autoSnap]);
  ...
  // Handle asset upload with progress tracking
    ...
    for (const file of Array.from(files)) {
      try {
        setUploadProgress({
          ...
          percent: 0,
          uploadedMB: 0,
          totalMB: Math.round(file.size / 1024 / 1024),
        });

        const newAsset = await uploadAsset(file, (percent, uploadedMB, totalMB) => {
          setUploadProgress({ filename: file.name, percent, uploadedMB, totalMB });
        });
  ...
  const handleRemoveDeadAir = useCallback(async (): Promise<{ duration: number; removedDuration: number }> => {
    ...
    const response = await fetch(`${FFMPEG_URL}/session/${session.sessionId}/remove-dead-air`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
  ...
  const handleTranscribeAndAddCaptions = useCallback(async (options?: { highlightColor?: string; fontFamily?: string }) => {
    ...
    const response = await fetch(`${FFMPEG_URL}/session/${session.sessionId}/transcribe`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleAddMotionGraphicFromPrompt = useCallback(async (config: {
    ...
    try {
      ...
      const response = await fetch(`${FFMPEG_URL}/session/${session.sessionId}/render-motion-graphic`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleCreateCustomAnimation = useCallback(async (description: string, startTime?: number, endTime?: number, attachedAssetIds?: string[], durationSeconds?: number) => {
    ...
    try {
      ...
      const response = await fetch(`${FFMPEG_URL}/session/${session.sessionId}/generate-animation`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          description,
  ...
  const handleAnalyzeForAnimation = useCallback(async (request: {
    ...
    const response = await fetch(`${FFMPEG_URL}/session/${session.sessionId}/analyze-for-animation`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
  ...
  const handleRenderFromConcept = useCallback(async (concept: {
    ...
    const response = await fetch(`${FFMPEG_URL}/session/${session.sessionId}/render-from-concept`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        concept,
  ...
  const handleGenerateTranscriptAnimation = useCallback(async () => {
    ...
    const response = await fetch(`${FFMPEG_URL}/session/${session.sessionId}/generate-transcript-animation`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        fps: 30,
  ...
  const handleGenerateBatchAnimations = useCallback(async (count: number) => {
    ...
    const response = await fetch(`${FFMPEG_URL}/session/${session.sessionId}/generate-batch-animations`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        count,
  ...
  const handleExtractAudio = useCallback(async () => {
    ...
    const response = await fetch(`${FFMPEG_URL}/session/${session.sessionId}/extract-audio`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        assetId: videoAsset.id,
  ...
  const handleCreateContextualAnimation = useCallback(async (request: {
    ...
    try {
      ...
      const response = await fetch(`${FFMPEG_URL}/session/${session.sessionId}/generate-contextual-animation`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          assetId: videoAsset.id,
  ...
  const handleEditAnimation = useCallback(async (
    ...
    const response = await fetch(`${FFMPEG_URL}/session/${session.sessionId}/edit-animation`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        assetId,
  ...
  return (
    <div className="flex flex-col h-screen bg-zinc-950 text-white overflow-hidden">
      ...
      <div className="flex flex-1 min-h-0">
        ...
        >
          <div className="h-full flex flex-col bg-zinc-900/80 backdrop-blur-sm">
            {/* Agent Tabs — order: Director, Clipify, DiCaprio, Picasso */}
            <div className="flex items-center gap-1 px-2 border-b border-zinc-800/50">
              ...
              <button
                onClick={() => setActiveAgent('clipify')}
                className={`flex items-center gap-1.5 px-3 py-2 text-sm font-medium transition-colors ${
                  activeAgent === 'clipify'
                    ? 'text-zinc-200 border-b-2 border-zinc-300 bg-zinc-800/30'
                    : 'text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800/20'
              ...
              <button
                onClick={() => setActiveAgent('hyperframes')}
                className={`flex items-center gap-1.5 px-3 py-2 text-sm font-medium transition-colors ${
                  activeAgent === 'hyperframes'
                    ? 'text-zinc-200 border-b-2 border-zinc-300 bg-zinc-800/30'
                    : 'text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800/20'
                }`}
              >
            ...
            <div className="flex-1 overflow-hidden relative">
              ...
              <div className={`absolute inset-0 ${activeAgent === 'clipify' ? '' : 'hidden'}`}>
                <ClipifyPanel
                  sessionId={session?.sessionId ?? null}
                  assets={assets}
                  onShortsGenerated={(assetIds) => {
                    console.log('Shorts generated:', assetIds);
                  }}
                  onRefreshAssets={refreshAssets}
                />
              </div>
              <div className={`absolute inset-0 ${activeAgent === 'hyperframes' ? '' : 'hidden'}`}>
                <HyperFramesPanel
      ...
      {/* Undo/Redo Toast */}
      {undoToast && (
        <div className="fixed bottom-20 left-1/2 -translate-x-1/2 z-50 px-4 py-2 bg-zinc-800/90 backdrop-blur-sm rounded-lg border border-zinc-700 text-sm text-zinc-200 shadow-xl transition-all animate-fade-in">
          {undoToast}
        </div>
      )}
      ...
      {showProgress && activeJob && (
        <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50 w-96 bg-zinc-900/95 backdrop-blur-sm border border-zinc-700 rounded-xl shadow-2xl p-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-medium text-zinc-300 uppercase tracking-wider">
              {activeJob.status === 'pending' ? 'Queued' : 'Processing'}
            </span>
            <button
              onClick={cancelJob}
          ...
          <div className="w-full h-1.5 bg-zinc-800 rounded-full overflow-hidden">
            <div
              ...
              style={{ width: `${Math.max(activeJob.status === 'pending' ? 5 : activeJob.percent, 2)}%` }}
            />
          </div>
          {activeJob.step && (
            <p className="text-xs text-zinc-500 mt-2 truncate">{activeJob.step}</p>
          )}
          {activeJob.status === 'running' && (
            <p className="text-xs text-zinc-600 mt-1">{Math.round(activeJob.percent)}%</p>
          )}
        </div>
      )}
      ...
      {showShortcutsHelp && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-zinc-900 rounded-xl border border-zinc-700 max-w-md w-full overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-zinc-700">
              <h2 className="text-lg font-semibold">Keyboard Shortcuts</h2>
              <button onClick={() => setShowShortcutsHelp(false)} className="p-1 hover:bg-zinc-700 rounded transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-4 space-y-2 text-sm">
              {[
                ...
                <div key={key} className="flex items-center justify-between py-1">
                  ...
                  <kbd className="px-2 py-0.5 bg-zinc-800 rounded text-xs font-mono text-zinc-300 border border-zinc-700">{key}</kbd>
                </div>
              ))}
            </div>
          </div>
